...
5. Confirm separate 7-day, 14-day, 28-day, and super-bonus milestones do not block each other.
...
9. Reset test data only after exporting a backup, and confirm the dashboard reloads with a reset-complete message and the family photo, child photos, and Gmail links are cleared with the rest of the local beta data.
...
The repository now includes Netlify Function scaffolding for Google session verification, parent-admin `family-settings` and `chore-library` saves, authenticated child contact settings, the `chore-record` completion/review guardrail, the `review-finalization` same-day noon review lock, the parent-admin `money-ledger` guardrail for fines and bonuses, Supabase Storage photo upload/record updates, extension request and approval texts, scheduled noon review texts, scheduled opted-in teen reminder texts, redo texts, Web Push subscriptions, the parent-admin `notification-log` audit reader, and Twilio SMS sends, but those functions require real Supabase, Twilio, and Web Push VAPID environment variables before they can be used on the hosted site.
