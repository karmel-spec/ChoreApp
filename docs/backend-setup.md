...
- `review-finalization`: lets signed-in family members reload finalized noon reviews, and lets only parent admins save same-day finalization locks so duplicate fines/reviews are blocked across devices.
- `chore-feedback`: lets signed-in family members read timing/difficulty feedback, lets children submit feedback for their own chores, and lets parent admins accept or deny feedback.
...
The `review-finalization` function is the production source for finalized noon review locks. It reloads recent review decisions before a parent finalizes a child, saves who finalized the date, stores missed chores, excused/hold status, charged amount, linked ledger entry, and blocks duplicate same-day finalization records with the `review_finalizations` unique child/date rule.
